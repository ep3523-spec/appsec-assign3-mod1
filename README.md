...
| Area | Change |
|------|--------|
| Secrets | Introduced Kubernetes Secret manifest `GiftcardSite/k8/django-secrets.yaml` holding base64 placeholders for `DJANGO_SECRET_KEY` and `MYSQL_ROOT_PASSWORD` (replace before use). |
| Deployments | Increased replicas for Django and MySQL from 1 to 2 (basic availability); environment variables now reference secret keys instead of inline plaintext. |
| CI/CD | Added workflow `.github/workflows/push-django.yml` to build & push image (requires self-hosted runner and DockerHub secrets). Added `.github/workflows/security-scan.yml` running Trivy for HIGH/CRITICAL severity prior to push. |
| Build Context | Added `.dockerignore` to prevent unnecessary files (and secrets) from entering Docker build context. |
| Commit Integrity | Added `docs/commit-signing.md` with GPG & SSH signing guidance to ensure commits can be verified. |

### GitHub Secrets Required
Create the following repository Actions secrets before running workflows:
* `DOCKERHUB_USERNAME`
* `DOCKERHUB_TOKEN` (DockerHub access token; least privilege)
* `DOCKER_REPO` (e.g. `assign3-django`)
* `DJANGO_SECRET_KEY` (output of Django's `get_random_secret_key()`)

### Replace Placeholder Secret Values
Run locally before applying manifests:
```bash
echo -n 'actual_django_secret' | base64
echo -n 'actual_mysql_root_password' | base64
# Edit GiftcardSite/k8/django-secrets.yaml with new base64 strings
...
Trigger via GitHub UI (Actions > Security Scan > Run workflow) after pushing changes; address HIGH/CRITICAL findings before tagging for grading.

### Notes on Autograder Compatibility
Replicas were increased; if autograder expects single replicas you may temporarily revert `replicas: 2` to `replicas: 1`. Secret-based env injection retains variable names used by application—should not break runtime. Ensure `DJANGO_SECRET_KEY` is present; container will fail without it.

### Tagging Reminder
Only tag after successful build, scan, and functional verification:
```bash
git tag -a -m "Completed assign3 module1." assign3mod1handin
git push origin main
git push origin assign3mod1handin
...
### Local Quick Test (Without Kubernetes Secrets)
You can still run locally with a temporary secret:
```bash
docker build -t local/assign3:dev .
docker run --rm -e DJANGO_SECRET_KEY="dev_temp_secret" -e MYSQL_HOST="localhost" -e MYSQL_ROOT_PASSWORD="dev_pwd" -p 8000:8000 local/assign3:dev
```

Keep secrets out of commits—use `.env` (excluded by `.gitignore`).
#   T r i g g e r   w o r k f l o w   r e - r u n  
 #   T r i g g e r   w i t h   u p d a t e d   t o k e n  
 